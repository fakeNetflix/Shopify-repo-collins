...
for i in `seq 300 310`; do
  ...
  curl --basic -X PUT -H "Accept: text/plain" -u collins:collins "${URL}/${TAG}"
  curl --basic -H "Accept: text/plain" -u collins:collins --data-urlencode "lldp@${LLDP_FILE}" --data-urlencode "lshw@${LSHW_FILE}" --data-urlencode 'CHASSIS_TAG=Testing this' "${URL}/${TAG}"
  sleep 10
done
